...
getPathSingle <- function(ToP, obs, obsTarget, doPredict) {
    ...
    while (leafNotReached) {
        ...
        # if doPredict==TRUE, make the prediction on this node ie if we want to get the predictions on the path
        if (doPredict) {
            pred <- predictNodeModel(Xnew = t(obs), posterior = node$model$posterior, blrResVariance = ToP$blrResVariance, bayesian = ToP$bayesian, 
        ...
        # Test if this node is a leaf ie it's a leaf
        if (node$isLeaf == 1) {
            leafNotReached <- FALSE
            
        } else {
            # it's not a leaf We get the index of the next node on the path
    ...
    return(base::list(path = path[!base::is.na(path)], predictions = predictions[!base::is.na(predictions)], variancePredictions = variancePredictions[!base::is.na(variancePredictions)], 
        target = obsTarget))
}

getPathMulti <- function(X, Y, ToP) {
    
    # We apply get_path_ind on each observation we get a list of nrow(df_obs)
    n <- base::nrow(X)
    ...
    for (idx in 1:n) {
        if (base::is.na(Y)[1]) {
            # if there is no target (prediction on new data)
            obsPaths[[obsPathsTracker]] <- getPathSingle(ToP = ToP, obs = X[idx, ], obsTarget = NA, doPredict = TRUE)
            
        } else {
            # ie if there is a target
            obsPaths[[obsPathsTracker]] <- getPathSingle(ToP = ToP, obs = X[idx, ], obsTarget = Y[idx], doPredict = TRUE)
        }
