    ...
    if(bayesian){
        ...
        if(!base::is.na(blrResVariance)){ 
            ...
            if(computeFullDistribution){
                Sigma_p <- 1/blrResVariance + base::diag(Xnew %*% posterior$variance %*% t(Xnew))
            } else{
                Sigma_p <- NA
            }
        ...
        # Model with unknown variance --------------
            ...
            if(computeFullDistribution){
                a_p <- 2 * posterior$a 
                Sigma_p <- posterior$b / posterior$a * (base::diag(n) + Xnew %*% posterior$V %*% t(Xnew))
            } else {
                a_p <- NA 
                Sigma_p <- NA
            }
...
computeMarginalLikelihood <- function(X, Y,
    ...
    if(!base::is.na(blrResVariance)){   
        ...
        if(n > maxSizeSlices){ # data too large to be evaluated in one time, we assume independance here
            ...
            for(i in 1:nbSlices){
                ...
                perf_i <- mvtnorm::dmvnorm(Y_i, 
                                           ...
                                           sigma = 1/blrResVariance * base::diag(endSlice - startSlice + 1) + X_i %*% S0 %*% base::t(X_i), 
                                           log=TRUE)
            ...
            perfModel <- mvtnorm::dmvnorm(Y, 
                                          mean = as.vector(X %*% m0), 
                                          sigma = 1/blrResVariance * base::diag(n) + X %*% S0 %*% base::t(X), 
                                          log=TRUE)
        }

        averagePerf <- perfModel / n # Average log marginal likelihood per observation 
        ...
        if(n>maxSizeSlices){ # data too large to be evaluated at one, we assume independance here
            nbSlices <- base::ceiling(n / maxSizeSlices)
            ...
            for(i in 1:nbSlices){
                ...
                sig0_i <- X_i %*% V0 %*% t(X_i)
                sig_i <- b0 / a0 * (base::diag(endSlice - startSlice + 1) + sig0_i)
...
trainNodeModel <- function(XtrainFull, YtrainFull,
    ...
    if(!bayesian){
        ...
        m1 <- lm$coefficients
        m1[base::is.na(m1)] <- 0 # We set the undefined coefficients (usually rank issue) to 0
      
        tXX <- base::t(Xtrain) %*% Xtrain
        
        S1 <- base::tryCatch( # For the cases where the variance matrix is not invertible
                { 
                    base::solve(blrResVariance * tXX)
                }, 
                error = function(error_condition){return(MASS::ginv(blrResVariance * tXX))}
                )   
    ...
    # Compute the BToPs model ------------------------
        ...
        if(!base::is.na(blrResVariance)){ 
            ...
            invS0 <- base::solve(S0)
            S1 <- base::solve(invS0 + blrResVariance * base::t(Xtrain) %*% Xtrain)
            m1 <- S1 %*% (blrResVariance * base::t(Xtrain) %*% Ytrain + invS0 %*% m0)
        ...
        # Model with unknown variance -------------------
            ...
            a1 <- a0 + n/2 
            b1 <- base::as.numeric(b0 + 1/2 * (base::t(m0) %*% invV0 %*% m0 + base::t(Ytrain) %*% Ytrain - base::t(m1) %*% invV1 %*% m1))
