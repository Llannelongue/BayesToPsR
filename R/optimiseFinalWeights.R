...
addPredVal2 <- function(Xval2, Yval2, ToP) {
    ...
    obsPaths <- getPathMulti(X = Xval2, Y = Yval2, ToP = ToP)
    ...
    for (pathInd in obsPaths) {
        ...
        ## we add the prediction to the corresponding node in ToP we collect the existing prediction tracker
    ...
    for (leaf in ToP$terminalLeaves) {
        ...
        if (weightsMethod == "lm") {
            ...
            if (nbObsV2 <= nbPredictors) {
                # ie if there is not enough observations to fit a lm (covariance matrix singular) We cannot fit a model for the weights, we leave
                # which_modelWeights = 'mean' For prediction, we'll use a default set of weights
                leafWeightsMethod <- "mean"
                
            } else {
                # We can fit a lm for weights We update the leaf with the matrix without NAs
                ...
                if (base::anyNA(coefs)) {
                  # if there is a singularity and some coef are not defined, we go back to the standard mean
                  leafWeightsMethod <- "mean"
                } else {
            ...
            if (nbObsV2 <= minNbObsV2) {
                # ie if there is not enough observations
                leafWeightsMethod <- "mean"
            } else {
                ## We compute the performance on the second validation set We update the leaf with the matrix without NAs
                ToP$Nodes[[leaf]]$predLabelVal2 <- ToP$Nodes[[leaf]]$predLabelVal2[1:nbObsV2, ]
                ...
                for (i in 1:(ncol(dfLeaf) - 1)) {
                  perfVal2[[i]] <- perfMetric(trueLabel = dfLeaf$Target, predLabel = dfLeaf[, i], metric = metric, display = FALSE)
                }
            ...
            ## We normalise the weights First we compute the log of the sum of the margina likelihoods
