...
computeOverallPredictor <- function(nodePreds, nodeVars, weights, bayesian) {
    ...
    return(list(predictedValue = predictedValue, predictedVariance = unname(predictedVariance)))
}

btopsPredict <- function(ToP, newdata) {
    
    # tic('Prediction')
    newdata2 <- base::cbind(Intercept = 1, btopsNormaliseNewdata(normalizer = ToP$normalizer, newdata = newdata, returnDf = FALSE))
    ...
    if (ToP$nbNodes == 1) {
        ...
        for (idx in 1:n) {
            pred <- predictNodeModel(Xnew = matrix(newdata2[idx, ], nrow = 1), posterior = ToP$Nodes[[1]]$model$posterior, blrResVariance = ToP$blrResVariance, 
                bayesian = ToP$bayesian, computeFullDistribution = TRUE)
        ...
        # ie if there is more than one node in the tree
        ...
        for (pathInd in obsPaths) {
            ...
            overallPred <- computeOverallPredictor(nodePreds = unlist(pathInd$predictions), nodeVars = pathInd$variancePredictions, weights = ToP$Nodes[[leaf]]$weights, 
    ...
    return(list(predictedValues = unlist(predictedValues), rawPredictedValues = rawPredictedValues, predictedVariances = unlist(predictedVariances), 
